...
internal class AndroidBackgroundMusicController(
    ...
    initialSettings: AndroidMusicSettings = AndroidMusicSettings(),
) : AutoCloseable {
    private var isGameStarted = false
    private var isForeground = false
    private var isClosed = false
    private var musicSettings = initialSettings.normalized()

    init {
        player.setVolume(musicSettings.effectiveVolume)
    }
    ...
    fun onMusicSettingsChanged(settings: AndroidMusicSettings) {
        if (isClosed) return

        musicSettings = settings.normalized()
        syncPlayback()
    }

    private fun syncPlayback() {
        val effectiveVolume = musicSettings.effectiveVolume
        player.setVolume(effectiveVolume)
        if (isForeground && isGameStarted && effectiveVolume > AndroidMusicSettings.MIN_VOLUME) {
            player.playLooping()
        } else {
            player.pause()
        }
...
internal class AudioFocusBackgroundMusicPlayer(
    ...
    override fun setVolume(volume: Float) {
        ...
        mediaPlayer.setVolume(volume.coerceIn(AndroidMusicSettings.MIN_VOLUME, AndroidMusicSettings.MAX_VOLUME))
    }
...
private class AndroidMediaPlayer(
    ...
    override fun setVolume(volume: Float) {
        ...
        val normalizedVolume = volume.coerceIn(AndroidMusicSettings.MIN_VOLUME, AndroidMusicSettings.MAX_VOLUME)

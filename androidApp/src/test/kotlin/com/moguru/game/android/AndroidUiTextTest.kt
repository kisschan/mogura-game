...
class AndroidUiTextTest {
    ...
    @Test
    fun `board highlight styles and selected move indicator stay visually distinct`() {
        ...
        val selectedMove = selectedMoveTargetIndicatorStyle()
        ...
        assertEquals(0.90f, selectedMove.cellScale)
        assertEquals(0xFFFFFFFF.toInt(), selectedMove.underStrokeArgb)
        assertEquals(0xFFD32F2F.toInt(), selectedMove.topStrokeArgb)
        assertEquals(7.dp, selectedMove.underStrokeWidth)
        assertEquals(4.dp, selectedMove.topStrokeWidth)
        assertEquals(12.dp, selectedMove.armLength)
        assertEquals(4.dp, selectedMove.haloStrokeWidth)
        assertEquals(0.55f, selectedMove.haloMinAlpha)
        assertEquals(1f, selectedMove.haloMaxAlpha)
        assertEquals(0.94f, selectedMove.haloMinScale)
        assertEquals(1f, selectedMove.haloMaxScale)
        assertEquals(800, selectedMove.pulseDurationMillis)
        assertFalse(move.strokeArgb == selectedMove.topStrokeArgb, "移動候補の青と選択先の赤を分離する")
        assertTrue(selectedMove.underStrokeWidth > selectedMove.topStrokeWidth)
        assertTrue(selectedMove.haloMinAlpha > 0f, "パルス中も選択表示を完全には消さない")
        assertTrue(selectedMove.haloMaxAlpha <= 1f)
        assertTrue(selectedMove.cellScale < selectedMove.haloMinScale, "固定枠とハローを形でも区別する")
        assertTrue(selectedMove.haloMinScale < selectedMove.haloMaxScale)
        assertTrue(selectedMove.haloMaxScale <= 1f, "ハローを選択マス内に収める")
        assertTrue(selectedMove.pulseDurationMillis >= 500)
        assertEquals("selected-move-target-2-1", selectedMoveTargetIndicatorTestTag(Position(2, 1)))
        assertEquals("selected-move-target-halo-2-1", selectedMoveTargetHaloTestTag(Position(2, 1)))

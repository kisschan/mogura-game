    ...
    val cells: Map<Position, Cell> = buildMap {
        for (row in 0 until ROWS) {
            for (col in 0 until COLS) {
                ...
                val type = when {
                    position in INVALID_POSITIONS -> CellType.INVALID
                    position in NEST_POSITIONS -> CellType.NEST
                    position in HOT_ZONE_POSITIONS -> CellType.HOT_ZONE
                    row == 0 -> CellType.GROUND
                    else -> CellType.UNDERGROUND
                }
                put(position, Cell(position, type))
            }
        }
    }

    /** 指定座標のセルを返す。盤面外なら `null`。 */
